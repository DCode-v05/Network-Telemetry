...
export function latency(rows, dets, anomalies) {
  ...
  anomalies.forEach((at) => {
    dets.forEach((d) => {
      const arr = acc[d + "|" + at];
      if (arr && arr.length) {
        out.push({ detector: d, anomaly: at, value: arr.reduce((a, b) => a + b, 0) / arr.length });
      }
    });
  });
  return out;
...
export function radar(rows, dets) {
  ...
  const out = {};
  dets.forEach((d) => { out[d] = norm(d); });
  return { raw, norm: out, axes: ["F1", "TPR", "Precision", "Det. Rate", "Low FPR"] };
}
