...
- Architecture + full results: `docs/ARCHITECTURE_AND_RESULTS.md` (regenerate via `scripts/gen_solution_md.py`)
- Dashboard: `dashboard/` (React+Vite+ECharts; `npm install` then `npm run build`)
- Tools: `scripts/eval_candidate.py` (evaluate any detector per anomaly type), `scripts/run_all.ps1` (full pipeline)
- Note: scratch (search candidates, probe scripts) and all caches/build artifacts were removed in the repo cleanup.
